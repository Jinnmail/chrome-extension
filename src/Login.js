  ...
  const onLoginClick = async (event) => {
    ...
    if (allowedToSubmit) {
      ...
      if (!json.error) {
        ...
        // localStorage.setItem('sessionToken', json.data.sessionToken);
        localStorage.setItem("isLogged", 1);
        chrome.storage.sync.set({ sessionToken: json.data.sessionToken, isLogged: 1, email: email }, function () {
    ...
    <form>
      <Grid container style={{textAlign: 'center'}}>
        ...
        <Grid item xs={12} style={{textAlign: "right"}}>
          <Button variant="contained" color="primary" fullWidth onClick={onLoginClick} disabled={!allowedToSubmit}>Log In</Button>
        </Grid>
